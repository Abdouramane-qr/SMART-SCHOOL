...
return new class extends Migration
{
    public function up(): void
    {
        ...
        DB::statement(<<<'SQL'
create or replace view v_admin_finance_monthly
...
select
    ...
    coalesce(sa.total, 0) as salaries_total
from schools s
join (
...
where current_setting('app.role', true) = 'admin'
  and current_setting('app.school_id', true)::bigint = s.id
SQL);

        DB::statement(<<<'SQL'
create or replace view v_admin_stats_summary
with (security_barrier = true) as
select
    s.id as school_id,
    count(distinct e.id) as students_total,
    count(distinct c.id) as classes_total,
    count(distinct t.id) as teachers_total,
    count(distinct a.id) as absences_total,
    coalesce(round(avg(n.value)::numeric, 2), 0) as avg_grade
from schools s
left join eleves e on e.school_id = s.id
left join classes c on c.school_id = s.id
left join enseignants t on t.school_id = s.id
left join absences a on a.school_id = s.id
...
SQL);
        ...
        DB::statement(<<<'SQL'
create or replace view v_school_users
with (security_barrier = true) as
select distinct su.school_id, su.user_id
from (
    select school_id, user_id from enseignants where user_id is not null
    union
    select school_id, user_id from eleves where user_id is not null
    union
    select school_id, user_id from parents where user_id is not null
    union
    select school_id, changed_by as user_id from audit_logs where changed_by is not null
) su
where current_setting('app.role', true) = 'admin'
  and current_setting('app.school_id', true)::bigint = su.school_id
SQL);

        DB::statement(<<<'SQL'
create or replace view v_admin_user_roles_summary
with (security_barrier = true) as
select
    su.school_id,
    r.name as role_name,
    count(distinct su.user_id) as users_total
from v_school_users su
join model_has_roles mhr on mhr.model_id = su.user_id and mhr.model_type = 'App\\Models\\User'
join roles r on r.id = mhr.role_id
where current_setting('app.role', true) = 'admin'
  and current_setting('app.school_id', true)::bigint = su.school_id
group by su.school_id, r.name
SQL);

        DB::statement(<<<'SQL'
create or replace view ai_documents with (security_barrier = true) as
select
    concat('admin-stats-', v.school_id) as doc_id,
    v.school_id as school_id,
    'admin' as rag_role,
    'school_stats' as document_type,
    concat(
        'School stats: students=', v.students_total,
        ', classes=', v.classes_total,
        ', teachers=', v.teachers_total,
        ', absences=', v.absences_total,
        ', avg_grade=', v.avg_grade
    ) as document_text,
    now() as updated_at,
    'schools' as source_table,
    v.school_id as source_id
from v_admin_stats_summary v

union all
select
    concat('admin-finance-', f.school_id, '-', to_char(f.month, 'YYYY-MM')) as doc_id,
    f.school_id as school_id,
    'admin' as rag_role,
    'finance_month' as document_type,
    concat(
        'Month ', to_char(f.month, 'YYYY-MM'),
        ': payments=', f.payments_total,
        ', expenses=', f.expenses_total,
        ', salaries=', f.salaries_total
    ) as document_text,
    now() as updated_at,
    'finance' as source_table,
    null::bigint as source_id
from v_admin_finance_monthly f

union all
select
    concat('admin-users-', u.school_id) as doc_id,
    u.school_id as school_id,
    'admin' as rag_role,
    'user_roles' as document_type,
    concat(
        'Users by role: ',
        coalesce(string_agg(u.role_name || '=' || u.users_total, ', '), 'none')
    ) as document_text,
    now() as updated_at,
    'users' as source_table,
    null::bigint as source_id
from v_admin_user_roles_summary u
...
SQL);
    ...
    public function down(): void
    {
        ...
        DB::statement('drop view if exists v_admin_user_roles_summary');
        DB::statement('drop view if exists v_school_users');
        DB::statement('drop view if exists v_admin_stats_summary');
        DB::statement('drop view if exists v_admin_finance_monthly');
    }
};
