...
import { laravelClassesApi, laravelStudentsApi } from "@/services/laravelSchoolApi";
...
export function AddStudentDialog({ isOpen, onClose, onSuccess }: AddStudentDialogProps) {
  ...
  const fetchClasses = async () => {
    try {
      const data = await laravelClassesApi.getAll();
      setClasses(data || []);
    } catch (error) {
  ...
  const onSubmit = async (values: z.infer<typeof studentSchema>) => {
    try {
      ...
      await laravelStudentsApi.create({
        ...
        classe_id: values.class_id || null,
      });
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Textarea placeholder="Adresse complète" {...field} rows={2} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
