...
export function EditTimetableDialog({
  ...
  useEffect(() => {
    if (entry) {
      setFormData({
        ...
        classroom_id: entry.classroom_id != null ? String(entry.classroom_id) : "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!entry || !formData.class_id || !formData.subject_id || !formData.day_of_week) {
      toast({
        title: "Erreur",
    ...
    setIsLoading(true);
    try {
      await laravelTimetableApi.update(entry.id, {
